...
class App extends Component {
  ...
  tick = () => {
    ...
    if (timer === 0) {
      ...
      this.setState(prev => ({gameOver: !prev.gameOver}))
      return
    }
  ...
  getFilterData = () => {
    ...
    const filterData = imagesList.filter(each => each.category === activeTab)
  ...
  onIconClick = id => {
    ...
    const findMatched = imagesList.filter(each => each.imageUrl === matchImg)
    if (findMatched[0].id === id) {
      this.setState(prev => ({score: prev.score + 1}))
      const matchShuffle = imagesList.sort(() => Math.random() - 0.5)
      this.setState({matchImg: matchShuffle[0].imageUrl})
    } else {
      clearInterval(this.timerId)
      this.setState(prev => ({gameOver: !prev.gameOver}))
    }
  ...
  onPlayAgain = () => {
    this.setState({score: 0, timer: 60, gameOver: false})
    this.componentDidMount()
  }

  render() {
    const {activeTab, matchImg, score, timer, gameOver} = this.state
    ...
    return (
      <div className="app-container">
        ...
        <div className="page-container">
          {gameOver ? (
            <GameOverCard onPlayAgain={this.onPlayAgain} score={score} />
          ) : (
            <div className="sections-container">
              <div>
                <img alt="match" className="match-img" src={matchImg} />
              </div>
